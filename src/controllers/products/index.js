  ...
  try {
    ...
    if (files && files.length === 2) {
      ...
      for (let i = 0; i < files.length; ++i) {
        fileNamesArr.push(`${files[i].filename}`);
      }
      // var newFileName1 = `${data["name"]}_image${Date
      //   .now()}_${Math.floor((Math.random()*Math.pow(36,9))+Date.now())}`;
      //       fs.renameSync(`./public/images/${files[0]?.filename}`,`./public/images/${newFileName1}`)
      //         console.log('New File Name',newFileName1,'Old File name:',files[0]?.filename )
      ...
      const product = await db("products")
        .insert({
          ...data,
          img_id: imagesData[0].id,
          img1_id: imagesData[1].id,
        })
        .returning(["*"]);
