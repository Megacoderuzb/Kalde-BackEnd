...
const getNews = async (req, res, next) => {
  try {
    const a = await db.select().from("prays");
    const prays = await db("prays")
      .leftJoin("images", "images.id", "prays.news_img_id")
      .select(
        "prays.id",
        //
        "prays.title_uz",
...
const patchNews = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    const existing = await db("prays").where({ id }).first();
...
const deleteNews = async (req, res, next) => {
  try {
    ...
    const del = await db("prays").where({ id }).returning(["*"]).del();
...
const getMain = async (req, res, next) => {
  res.status(200).json({
    BackEnd_Dasturchi: "Muhammadjon Abduvaxobov",
  });
};
